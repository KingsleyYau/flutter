...
//        LSLCUserItem* userItem = m_userMgr->GetUserItem(inUserId);
//        if (NULL == userItem) {
//            FileLog("LiveChatManager", "LiveChatManager::OnSendScheduleInvite() getUserItem fail, fromId:%s", inUserId.c_str());
//            return;
//        }
//
//        userItem->m_inviteId = inviteId;
...
//        // 生成MessageItem
//        item = new LSLCMessageItem;
//        FileLog("LiveChatManager", "LiveChatManager::OnSendScheduleInvite() item:%p", item);
//        FileLog("LiveChatManager", "LiveChatManager::OnSendScheduleInvite() inviteId:%s", userItem->m_inviteId.c_str());
//        item->Init(m_msgIdBuilder.GetAndIncrement()
//                   , SendType_Recv
//                   , inUserId
//                   , m_userId
...
//
//        if (NULL != item) {
//            FileLevelLog("LiveChatManager", KLog::LOG_WARNING, "LiveChatManager::OnSendScheduleInvite() callback, item:%p", item);
//
//            // callback
//            //m_listener->OnRecvMessage(item);
//            // 为了区分OnRecvMessage,这个使用在Qn冒泡跳转到直播后，发送邀请语成功，如果用OnRecvMessage，直播会再冒泡 (Alex, 2019-07-26)
...
//        }
    ...
    if (NULL != msgItem && NULL != msgItem->GetScheduleInviteItem() && msgItem->GetScheduleInviteItem()->m_type != SCHEDULEINVITE_PENDING) {
